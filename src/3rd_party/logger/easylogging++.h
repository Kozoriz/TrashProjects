...
class Str : base::StaticClass {
 public:
  ...
  static inline std::string& rtrim(std::string& str) {
    str.erase(
        std::find_if(
            str.rbegin(), str.rend(), [](char c) { return !std::isspace(c); })
            .base(),
        str.end());
    return str;
  }
...
class Configurations
 ...
 public:
  ...
  /// true. If you dont do this explicitly (either by calling this function or
...
#if defined(ELPP_NO_LOG_TO_FILE)
    unsafeSetIfNotExist(
        Level::Global, ConfigurationType::Enabled, std::string("false"));
#else
    unsafeSetIfNotExist(
 ...
 public:
  ...
  inline bool installLogDispatchCallback(const std::string& id) {
    return base::utils::Utils::installCallback<
        T,
        base::type::LogDispatchCallbackPtr>(id, &m_logDispatchCallbacks);
  }
...
ELPP_INIT_EASYLOGGINGPP(new el::base::Storage(el::LogBuilderPtr( \
    new el::base::DefaultLogBuilder()), new el::base::AsyncDispatchWorker()))
